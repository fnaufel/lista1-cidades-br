...
1. Compare com a distribuição de agências públicas.
   ...
   Vamos sobrepor os histogramas:
    ...
    cidades %>% 
      ...
      ggplot() +
        geom_histogram(
          aes(x = log_publ, fill = 'publ'),
          breaks = 0:10,
          alpha = .2
        ) +
        geom_histogram(
          aes(x = log_priv, fill = 'priv'),
          breaks = 0:10,
          alpha = .3
        ) +
        scale_x_continuous(
   ...
   Ou seja, [cerca de $`r (100 * 2233/nrow(cidades)) %>% fm(0)`\%$ de cidades sem agências bancárias, nem privadas, nem públicas!]{.hl}
