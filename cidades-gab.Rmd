...
1. Faça um histograma da quantidade de agências bancárias públicas. Use o número de classes que você achar mais adequado.
   ...
   Criando uma nova coluna:
    ...
    cidades %>% 
      filter(agencias_publ > 0) %>% 
      mutate(log_publ = log(agencias_publ)) %>% 
      ggplot() +
        geom_histogram(
          aes(x = log_publ),
          breaks = 0:8
        ...
        labs(
          title = 'Quantidade de agências públicas (log)',
          x = TeX('$\\log_{10}$ agências'),
          y = NULL
        )
    ```
   
   Ou usando a coluna original, mudando a escala do gráfico:
    ...
    cidades %>% 
      ...
      ggplot() +
        geom_histogram(
          aes(x = agencias_publ),
          breaks = c(1:10, 20, 50, 100, 500, 1000)
        ) +
        scale_x_log10(
          breaks = c(1:10, 20, 50, 100, 500, 1000)
        ) +
   ...
   Perceba que, neste último gráfico, [os rótulos do eixo $x$ são as quantidades de agências, não os logaritmos, mas o espaçamento é logarítmico]{.hl}. Além disso, escolhi manualmente os limites das barras.
   
   Qual dos dois gráficos você acha melhor?
...
1. Faça um histograma da quantidade de agências bancárias privadas. Use o número de classes que você achar mais adequado.
   ...
   Criando uma nova coluna:
    ...
    cidades %>% 
      filter(agencias_priv > 0) %>% 
      mutate(log_priv = log(agencias_priv)) %>% 
      ggplot() +
        geom_histogram(
          aes(x = log_priv),
          breaks = 0:8
        ...
        labs(
          title = 'Quantidade de agências privadas (log)',
          x = TeX('$\\log_{10}$ agências'),
          y = NULL
        )
    ```
...
1. Compare com a distribuição de agências públicas.
   ...
   Na verdade, é mais fácil comparar as tabelas do que os histogramas:
   ...
   o que dá cerca de $`r (100 * 2233/nrow(cidades)) %>% fm(0)`\%$ de cidades sem agências bancárias, nem privadas, nem públicas!
