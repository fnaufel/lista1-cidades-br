...
1. Quais as $10$ cidades que têm a [maior quantidade de carros por habitante]{.hl}?
   ...
   As duas primeiras maneiras dão o mesmo resultado:
   
    ```{r}
    all(carros_pop_regular$cidade == carros_pop_resid$cidade)
    ```
   
   Usando a população estimada, não há nenhuma cidade em comum com os resultados das duas primeiras maneiras:
   
    ```{r}
    intersect(carros_pop_regular$cidade, carros_pop_est$cidade)
    ```
...
1. Quais as $10$ cidades que têm a [menor quantidade de carros por habitante]{.hl}?
   ...
   Vamos comparar:
    ...
    ```{r}
    tibble(
      regular = carros_pop_regular$cidade,
      resid = carros_pop_resid$cidade,
      estim = carros_pop_est$cidade
    )
    ```
...
1. Faça um histograma das áreas das cidades. Use o número de classes que você achar mais adequado.
   ...
   Podemos examinar apenas as cidades com área entre $100$ e $1000$ km² (com escala linear):
   
    ```{r}
    cidades %>% 
      filter(between(area, 100, 1000)) %>% 
      ggplot() +
        geom_histogram(
          aes(area),
...
1. Comente os resultados.
   ...
   As atividades associadas ao maior PIB *per capita* [mediano]{.hl} são (1) eletricidade, gás, água e esgoto e (2) comércio e reparação de veículos.
   
   As atividades associadas ao menor PIB *per capita* [mediano]{.hl} são administração, defesa, educação e saúde.
